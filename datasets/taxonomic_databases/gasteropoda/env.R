...
qsqs <- 0.5 #set quorum value for sqs subsampling
...
#taxonomic database
taxdb <- read.csv("marine gastropods - pbdb_data.csv", sep = ",", 
                  na.strings = "", stringsAsFactors = FALSE, skip = 21) #load taxonomic database
names(taxdb)[6] <- "genus"
...
#remove trend
env$T.scotese    <- c(NA,diff(env_detrend$T.scotese, differences = 1))
env$S.macarthur  <- c(NA,diff(env_detrend$S.macarthur, differences = 1))
env$Sf.paytan    <- c(NA,diff(env_detrend$Sf.paytan, differences = 1))
